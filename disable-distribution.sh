...
jq '.DistributionConfig.Enabled = false' dist-config.json | jq '{DistributionConfig: .DistributionConfig}' > disabled-config.json
...
echo "aws cloudfront get-distribution --id E1BU4CWN6QEQ6X --query 'Distribution.Status'"
echo ""
echo "Once the status shows 'Deployed', you can delete it with:"
echo "NEW_ETAG=\$(aws cloudfront get-distribution-config --id E1BU4CWN6QEQ6X --query ETag --output text)"
echo "aws cloudfront delete-distribution --id E1BU4CWN6QEQ6X --if-match \$NEW_ETAG"
